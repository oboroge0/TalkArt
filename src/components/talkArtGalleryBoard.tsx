...
export const TalkArtGalleryBoard: React.FC<TalkArtGalleryBoardProps> = ({
  ...
  const [canvasMode, setCanvasMode] = useState(false) // Toggle between HTML and Canvas
  ...
  if (canvasMode) {
    return (
      <TalkArtGalleryCanvas
        ...
        onSwitchToHtml={() => setCanvasMode(false)}
      />
    )
  }

  // HTML mode rendering - wrapper component to avoid hooks rules violation
  return (
    <TalkArtGalleryBoardHTML
      onClose={onClose}
      onSelectArtwork={onSelectArtwork}
      shouldRefresh={shouldRefresh}
      onRefreshComplete={onRefreshComplete}
      onSwitchToCanvas={() => setCanvasMode(true)}
    />
  )
}

// Separate component for HTML implementation
const TalkArtGalleryBoardHTML: React.FC<
  TalkArtGalleryBoardProps & {
      ...
      <div className="relative bg-amber-800/10 backdrop-blur-sm p-4 border-b-4 border-amber-700/20 shadow-lg">
        ...
        <div className="max-w-7xl mx-auto mt-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-6 py-2 rounded-full font-medium shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center gap-2"
          >
            <svg
              className="w-5 h-5"
              fill="none"
