...
import { TalkArtForm } from '@/components/talkArtForm'
import { TalkArtAudioControl } from '@/components/talkArtAudioControl'
...
const Home = () => {
  ...
  return (
    <div className="h-[100svh] bg-red-500" style={{ backgroundImage: bgUrl }}>
      <Meta />
      {!isTalkArtMode && <Introduction />}
      {modelType === 'vrm' ? <VrmViewer /> : <Live2DViewer />}
      {isTalkArtMode ? <TalkArtForm /> : <Form />}
      ...
      <WebSocketManager />
      <YoutubeManager />
      {!isTalkArtMode && <CharacterPresetMenu />}
    </div>
  )
}
