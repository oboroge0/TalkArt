...
import { GalleryState, GalleryItem, GeneratedArtwork } from '@/features/talkart/types'
  ...
  refreshGallery: async () => {
    ...
    try {
      ...
      if (data.success && data.artworks) {
        const galleryItems: GalleryItem[] = data.artworks.map((artwork: GeneratedArtwork, index: number) => ({
          id: `gallery_${artwork.id}`,
          artwork,
          position: generateRandomPosition(),
          displayOrder: index
